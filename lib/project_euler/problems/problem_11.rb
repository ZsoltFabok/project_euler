  ...
  class Problem11
    ...
    def self.execute
      grid = "08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08
              ...
              20 73 35 29 78 31 90 01 74 31 49 71 48 86 81 16 23 57 05 54
              01 70 54 71 83 51 54 69 16 92 33 48 61 43 52 01 89 19 67 48"
      new(Common::Arrays.new).calculate(grid)
    end

    def calculate(grid)
      max = 0
      grid = string_grid_to_array(grid)
    ...
    private
    def string_grid_to_array(grid_as_string)
      grid_ = grid_as_string.split(" ")
      dimension = Math.sqrt(grid_.size)
      grid = []
      (0..grid_.size - 1).step(dimension).each do |number|
        grid << array_of_strings_to_i(grid_[number..number + dimension - 1])
      end
      grid
    end

    def array_of_strings_to_i(array)
      array.map {|n| n.to_i}
    end
